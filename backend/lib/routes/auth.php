...
function register_auth_routes(FastRoute\RouteCollector $r, \Lib\Database $db, $user, \Lib\MailerFactory $mailer_factory, string $url, $mail_targets) {
    ...
    $r->addRoute('POST', 'api/mass-reset', function($_, $values) use ($db, $user, $mailer_factory, $mail_targets) {
        ...
        for($i = 0; $i < count($users); $i++) {
            ...
            $body .= "<p>E-mail: " . htmlspecialchars($user['email']) . "<br />
            ...
            try {
                ...
                error_log("Failed to send mail to $mail: $message");
            }

            // Wait 1 minute every 50
            if(($i + 1) % 50 == 0) {
                sleep(60);
            }
        }
